...
async def _update_netbox(session: aiohttp.ClientSession, response: Response) -> None:
    ...
    m = re.search(r"Lo0\s+up\s+up\s+(?P<description>\S+)", response.result)
    if m is None:
...
async def update_netbox(queue: asyncio.Queue) -> None:
    log("update_netbox - ждем сбора данных со всех устройств")
    await COLLECTION_DONE.wait()
    log("update_netbox - сбор данных со всех устройство завершен")
