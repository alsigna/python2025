...
async def main(coro_count: int) -> None:
    # # базовый пример
    # # оборачиваем код в async with asyncio.timeout(), все, что не успело выполнится,
    ...
    # # reschedule
    # # ждем выполнение coro(1), она укладывается в исходный 1.5c таймаут, потом обновляем новый таймаут на 1.5с
    # # что бы coro(2) успела завершиться
    # tasks = [asyncio.create_task(coro(i), name=f"task-{i}") for i in range(1, coro_count + 1)]
    # try:
    #     async with asyncio.timeout(1.5) as timeout:
    #         await tasks[0]
    #         timeout.reschedule(asyncio.get_running_loop().time() + 1.5)
    #         log(f"таймаут обновлен, текущее значение: {timeout.when() - asyncio.get_running_loop().time():.4f}")
    #         await asyncio.gather(*tasks, return_exceptions=True)
    # except TimeoutError:
    #     log("таймаут выполнения задач")
    # print_info(tasks)
    ...
    # # cancel
    # # отмена таймаута, ждем coro(1), и после этого отменяем, что фактически значит работу без таймаута
    # tasks = [asyncio.create_task(coro(i), name=f"task-{i}") for i in range(1, coro_count + 1)]
    # try:
    #     async with asyncio.timeout(2.5) as timeout:
    #         await tasks[0]
    ...
    # remain
    # позволяет контролировать, сколько времени осталось до конца таймаута
    tasks = [asyncio.create_task(coro(i), name=f"task-{i}") for i in range(1, coro_count + 1)]
    try:
        async with asyncio.timeout(1.5) as timeout:
