...
> [!note]
> Для некоторых семантически совместимых типов реализованы некоторые операции, например:
>
> ```python
> print("Hi" * 3) # HiHiHi - работает за счет перегрузки оператора, str.__mul__ метод
> print(True + 41) # 42 - работает за счет наследования, issubclass(bool, int)
> ```
...
- stubgen пытается получить типы из реализации, но зачастую используется Any или Incomplete
