...
async def main():
    # to_thread использует глобальный executor, который создает loop
    # количество потоков зависит от числа ядер, но можно переопределить
    # этот глобальный executor своим
    ...
    tasks = [
        ...
        # asyncio.create_task(asyncio.to_thread(blocking_io, 5), name="task-5"),
        # asyncio.create_task(asyncio.to_thread(blocking_io, 6), name="task-6"),
        # asyncio.create_task(asyncio.to_thread(blocking_io, 7), name="task-7"),
    ]
