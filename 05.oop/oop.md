...
5. магическое имя `__var__`: используется в Python для определения специальных служебных методов/переменных (магические/dunder методы). Не следует создавать свои объекты с такими именами.
