...
class FullDeviceTestCase(TestCase):
    ...
    def test_get_version_correct(self, mock_scrapli: MagicMock) -> None:
        mock_cli = MagicMock()
        # mock_cli = Response().result
        mock_cli.send_command.return_value.result = ", Version 17.3.3, RELEASE SOFTWARE (fc7)"
        mock_cli.send_command.return_value.failed = False
    ...
    def test_get_version_error(self, mock_scrapli: MagicMock) -> None:
        mock_cli = MagicMock()
        # mock_cli = Response().failed
        mock_cli.send_command.return_value.failed = True
        mock_scrapli.return_value = mock_cli
...
class MethodDeviceTestCase(TestCase):
    def test_get_version(self) -> None:
        ...
        with patch.object(
            ...
            return_value=type(
                "Response",
                (),
                {"result": ", Version 17.3.3, RELEASE SOFTWARE (fc7)", "failed": False},
            ),
        ):
            version = device.get_version()
            expected_version = "17.3.3"
    ...
    def test_get_version_error(self) -> None:
        device = Device("192.168.122.101")
        with patch.object(
            target=device.cli,
            attribute="send_command",
            ...
            return_value=type(
                "Response",
                (),
                {"result": ", Version 17.3.3, RELEASE SOFTWARE (fc7)", "failed": True},
            ),
        ):
            version = device.get_version()
            expected_version = "ERROR"
